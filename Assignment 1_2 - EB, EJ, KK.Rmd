...
>>>>>>> 7cf9bc6df7ba7bd629345015a689e92b94c695b2
kable(head(data))
```
...
Our dependent variable for this analysis is:

* % of residents within a particular county who voted Republican in the last presidential election
...
* Percentage of population identifying as Black / African-American only 
* Urban-Rural County classification

We are looking at these variables across the county level. 

## Regression on Continuous Variables

# Population Density

```{r, calculating correlations between continuous variables}
...
```{r, calculating the same thing with a regression analysis}
dens_model <- lm(pct_GOP ~ pop_dens, data = data)
summary(dens_model)
```
#  Median Income 

```{r, correlation test AND regression for median income and % GOP votes}
...
```{r, correlation test AND regression for % white-only and % GOP votes}
...
```{r, correlation test AND regression for black-only and % GOP votes}
...
^^^ I AM STRUGGLING TO INTERPRET THE ABOVE - ANY IDEAS????
>>>>>>> 7cf9bc6df7ba7bd629345015a689e92b94c695b2
