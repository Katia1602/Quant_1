...
export_summs(full_model, centered_model, 
             ...
             coefs = coeff_names)
```
**The model fit (given by the R-square value) is exactly the same, and the coefficients are all the same, except for the intercept

#Scaling

```{r}
centered_data_scaled <- centered_data %>%
  mutate(pct_GOP = pct_GOP * 100,
...
centered_model_scaled <- lm(pct_GOP ~ med_incE + pop_dens_100 + type, centered_data_scaled)
