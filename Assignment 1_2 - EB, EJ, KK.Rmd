...
interaction_model <- lm(pct_GOP ~ med_incE + pop_dens_100 + white_pct + black_pct + type + pop_dens_100*black_pct, centered_data_scaled)
...
The model fit did not change, as the R squared value is still the same. 
...
interact_plot(interaction_model, pred = med_incE, modx = black_pct,
  ...
  scale_x_continuous(breaks = breaks <- c(-2, -1, 0, 1, 2),
                     labels = round(breaks * 10 + mean(data$med_incE, na.rm = TRUE)),
                     name = "Median income") +
